...
class ResNet(nn.Module):
    def __init__(self, block, layers, mcfg):
        ...
        self.dwstem = nn.Sequential(
            nn.Conv2d(3, mcfg.nf//2, kernel_size=3, stride=2, padding=1, bias=False),
            self._norm_layer(mcfg.nf//2),
            nn.ReLU(inplace=True),

            nn.Conv2d(mcfg.nf//2, mcfg.nf//2, kernel_size=3, stride=1, padding=1, groups=mcfg.nf//2, bias=False),
            nn.Conv2d(mcfg.nf//2, mcfg.nf, kernel_size=1, stride=1, padding=0, bias=False),
    ...
    def _make_layer(self, block, inplanes, outplanes, num_block, stride=1, dilate=False, multi_grid=False):
        ...
        layers.append(
            block(inplanes, 
                  outplanes, 
                  expansion=self.bttln_exp,
                  stride=stride,
                  downsample=downsample,
                  groups=self.groups,
                  base_width=self.base_width,
                  dilation=previous_dilation * grids[0],
